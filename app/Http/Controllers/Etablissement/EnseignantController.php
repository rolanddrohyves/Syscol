...
class EnseignantController extends Controller
{
    ...
    public function create()
    {
        ...
        // ✅ CORRIGÉ : Récupérer TOUTES les matières (pas de filtre etablissement_id)
    ...
    public function store(Request $request)
    {
        ...
        // VÉRIFICATION : S'assurer que toutes les classes appartiennent à l'établissement
        $classesInvalides = [];
        ...
        // ✅ CORRIGÉ : Vérifier que les matières existent (sans filtre etablissement_id)
        if ($request->has('matieres')) {
            $matieresValides = Matiere::whereIn('id', $request->matieres)->count();
            if ($matieresValides != count($request->matieres)) {
                return redirect()->back()
                    ->withInput()
                    ->withErrors([
                        'matieres' => 'Certaines matières sélectionnées n\'existent pas.'
                    ]);
            }
        }
    ...
    public function emploiTemps($id)
    {
        ...
        // Statistiques pour l'emploi du temps
        $stats = [
