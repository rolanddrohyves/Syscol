{{-- resources/views/cpe/sanctions/create.blade.php --}}
@extends('layouts.app')

@section('title', 'Ajouter une sanction - CPE')
@section('page-title', 'Ajouter une sanction disciplinaire')
...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-2xl shadow-sm p-8">
        ...
        <div class="flex items-center space-x-4 mb-8 pb-6 border-b border-gray-100">
            ...
            <div>
                <h2 class="text-2xl font-bold text-gray-800">Ajouter une sanction</h2>
                <p class="text-gray-500">Enregistrer une sanction disciplinaire pour un élève</p>
            </div>
        </div>

        <!-- Formulaire -->
        <form action="{{ route('cpe.sanctions.store') }}" method="POST" class="space-y-8">
            @csrf
            ...
            <div class="space-y-6 pt-6 border-t border-gray-100">
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="motif" 
                              ...
                              placeholder="Raison de la sanction...">{{ old('motif') }}</textarea>
                    @error('motif')
                        <p class="text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="statut" 
                            ...
                            @foreach($statuts as $value => $label)
                                <option value="{{ $value }}" {{ old('statut', 'en_cours') == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
            ...
            <!-- Période d'application (si applicable) -->
            <div class="space-y-6 pt-6 border-t border-gray-100" id="periodeSection" style="display: none;">
                <h3 class="text-lg font-semibold text-gray-800 flex items-center">
            ...
            <div class="space-y-4 p-4 bg-purple-50 rounded-xl">
                ...
                <p class="text-xs text-purple-700">
                    La sanction sera enregistrée et visible dans le tableau de bord. 
                    Vous pourrez modifier son statut ultérieurement (en cours, exécutée, annulée).
                </p>
            </div>
...
<script>
    ...
    document.getElementById('type').addEventListener('change', function() {
        ...
        // Afficher la période pour les exclusions
    ...
    window.addEventListener('DOMContentLoaded', function() {
        ...
        // Vérifier le type au chargement pour afficher la période
        const typeSelect = document.getElementById('type');
        if (typeSelect.value === 'exclusion_temporaire' || typeSelect.value === 'exclusion_definitive') {
            document.getElementById('periodeSection').style.display = 'block';
        }
    });
