...
@section('title', 'Modifier un établissement - SYSCOL')
...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-2xl shadow-sm p-8">
        ...
        <div class="flex items-center space-x-4 mb-8 pb-6 border-b border-gray-100">
            ...
            <div>
                <h2 class="text-2xl font-bold text-gray-800">Modifier {{ $etablissement->nom }}</h2>
                <p class="text-gray-500">Modifiez les informations de l'établissement</p>
            </div>
        </div>

        <!-- Formulaire -->
        <form action="{{ route('admin.etablissements.update', $etablissement->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="type" 
                            ...
                            <option value="Primaire" {{ old('type', $etablissement->type) == 'Primaire' ? 'selected' : '' }}>Primaire</option>
                            <option value="Collège" {{ old('type', $etablissement->type) == 'Collège' ? 'selected' : '' }}>Collège</option>
                            <option value="Lycée" {{ old('type', $etablissement->type) == 'Lycée' ? 'selected' : '' }}>Lycée</option>
                            <option value="Primaire/Secondaire" {{ old('type', $etablissement->type) == 'Primaire/Secondaire' ? 'selected' : '' }}>Primaire/Secondaire</option>
                        </select>
                        @error('type')
                            <p class="text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex items-center space-x-4">
                            <div class="w-20 h-20 bg-gray-100 rounded-lg flex items-center justify-center border-2 border-dashed border-gray-300 overflow-hidden" id="logoPreview">
                                @if($etablissement->logo)
                                    <img src="{{ Storage::url($etablissement->logo) }}" class="w-full h-full object-cover">
                                @else
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-xs text-gray-400 mt-1">JPG, PNG ou GIF (max. 2MB). Laissez vide pour conserver le logo actuel.</p>
                            </div>
                        </div>
                        @error('logo')
                            <p class="text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="col-span-2 space-y-2">
                        ...
                        <textarea name="adresse" 
                                  ...
                                  placeholder="Adresse complète">{{ old('adresse', $etablissement->adresse) }}</textarea>
                        @error('adresse')
                            <p class="text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="space-y-6 pt-6 border-t border-gray-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" 
                               name="academie" 
                               value="{{ old('academie', $etablissement->academie) }}"
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" 
                               name="inspectorat" 
                               value="{{ old('inspectorat', $etablissement->inspectorat) }}"
            ...
            <div class="space-y-6 pt-6 border-t border-gray-100">
                <div class="flex items-center space-x-3">
                    <input type="checkbox" 
                           ...
                           id="is_active" 
                           value="1"
                           {{ old('is_active', $etablissement->is_active) ? 'checked' : '' }}
