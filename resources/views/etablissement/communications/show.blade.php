{{-- resources/views/etablissement/communications/show.blade.php --}}
@extends('layouts.app')
    ...
    <div class="bg-white rounded-2xl shadow-sm overflow-hidden">
        ...
        <div class="p-6">
            ...
            <div class="border-b border-gray-200 pb-4 mb-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="font-medium text-gray-800">{{ $message->sender->name ?? 'Inconnu' }}</p>
                        <p class="text-xs text-gray-400">{{ $message->sender->email ?? '' }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <span class="inline-flex items-center px-2 py-1 rounded text-xs font-medium {{ $message->lu ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                            ...
                            {{ $message->lu ? 'Lu le ' . ($message->lu_at ? $message->lu_at->format('d/m/Y H:i') : '') : 'Non lu' }}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="border-t border-gray-200 pt-4 mb-4">
                ...
                <form action="{{ route('etablissement.communications.reply', $message->id) }}" method="POST">
                    @csrf
            ...
            <div class="flex justify-between items-center pt-4 border-t border-gray-200">
                ...
                @if($message->type == 'parent_to_admin')
                <form action="{{ route('etablissement.communications.transfer', $message->id) }}" method="POST" class="inline">
                    @csrf
                    <button type="button" onclick="confirmTransfer(this.form)" 
                            class="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">
                        <i class="fas fa-share mr-2"></i>Transférer au Super Admin
                    </button>
                </form>
                @endif
                
                <form action="{{ route('etablissement.communications.destroy', $message->id) }}" method="POST" class="inline">
                    @csrf
                    @method('DELETE')
...
function confirmTransfer(form) {
    if(confirm('Transférer ce message au Super Administrateur ?')) {
        form.submit();
    }
}
</script>
